...
{-
isOrd2 = cataBTree g
	where g = split (either (const (Empty,True)) k2) (either (const True) h2) 
 	      compareNodes (a,((t1,b1),(t2,b2))) = (either (const True) ((< a).p1) (outBTree t1)) && (either (const True) ((> a).p1) (outBTree t2)) 
	      f2 a = (uncurry (&&)) . (compareNodes >< id) . a
              k2 = f2 . p1
              f a = split id (uncurry(&&) . (a >< a) . p1)
              h2 = f . p2 
-}

isOrd1 :: (Ord a) => BTree a -> Bool
isOrd1 Empty = True
isOrd1 (Node (a,(t1,t2))) = f (auxOrd (Node (a,(t1,t2))))
	where f =  (uncurry (&&)) . (funcaoComparacao >< id) 

auxOrd Empty = (Empty,True)
auxOrd (Node (a,(t1,t2))) = ((Node (a,(t1,t2))), isOrd1 t1 && isOrd1 t2) 
